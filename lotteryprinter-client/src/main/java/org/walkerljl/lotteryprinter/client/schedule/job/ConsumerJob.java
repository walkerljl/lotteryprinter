...
public class ConsumerJob implements Runnable {
	 ...
	 * @param printQueue
	 */
	public ConsumerJob(PrintService printService, Scheduler scheduler) {
			...
			while (isContinue) {
				...
				try {
					...
					logger.info(info.append("[")
							...
							.append("]取得任务[").append(task.getId()).append("]")
							.toString());
				} catch (Exception ex) {
					errorCounter++;
					if (errorCounter <= 10) {
